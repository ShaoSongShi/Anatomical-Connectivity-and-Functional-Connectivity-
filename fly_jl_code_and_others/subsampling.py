...
def fit_power_law_eigenvalues_start_end(eigenvalues, start_rank=1, end_rank=10, num_top_eigenvalues=None):
    """
    Fits a power law (log-log linear fit) to eigenvalues in the rank range [start_rank, end_rank].
    ...
    Args:
        ...
        start_rank (int): The starting rank (1-indexed) to include in the fit. Default 1.
        end_rank (int): The ending rank (1-indexed) to include in the fit. Default 10.
        num_top_eigenvalues (int, optional): [Legacy] If provided, overrides start_rank=1 
                                             and end_rank=num_top_eigenvalues. 
                                             Default None.
    ...
    Returns:
        dict: A dictionary containing:
            ...
            - 'data_y': The eigenvalues used for the fit.
            - 'fitted_y': The fitted values projected back to the original scale.
            - 'start_rank': Actual start rank used.
            - 'end_rank': Actual end rank used.
    """
    ...
    if num_top_eigenvalues is not None:
        # Legacy mode: fit top 1 to num_top_eigenvalues
        start_rank = 1
        end_rank = num_top_eigenvalues
    
    # Clamp bounds to valid range
    start_rank = max(1, start_rank)
    end_rank = min(total_count, end_rank)
    
    if start_rank > end_rank:
        raise ValueError(f"start_rank ({start_rank}) cannot be greater than end_rank ({end_rank}).")
    
    n_fit = end_rank - start_rank + 1
    if n_fit < 2:
        raise ValueError(f"At least 2 eigenvalues are required for a linear fit. Got {n_fit}.")
    
    # Extract the specific range of eigenvalues (e.g., 5th to 15th largest)
    top_evals = sorted_evals[start_rank-1:end_rank]
    print(f"Fitting eigenvalues from rank {start_rank} to {end_rank} (total {n_fit} points).")
    print(top_evals)
    
    # Check for non-positive eigenvalues which cannot be log-transformed
    if np.any(top_evals <= 0):
        raise ValueError("Selected eigenvalues must be strictly positive for a log-log fit.")
    
    # Calculate Normalized Rank (using global total count for normalization)
