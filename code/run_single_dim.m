function run_single_dim(input_path, output_path, dim_val)
    ...
    % 正确写法：先用圆括号取出 1×1 cell，再解引用
    D_cell = m.D_cells(idx, 1);   % 或 m.D_cells(idx) 如果 D_cells 是列向量
    D = D_cell{1};
    D = (D + D') / 2;
    n_obs = size(D,1);
    D(1:n_obs+1:end) = 0;
    ...
    for i = 1:length(num_fields)
        f = num_fields{i};   % 显式取出字符串
        if ~isfield(saved, f) || isempty(saved.(f))
            saved.(f) = nan(n_dims, 1);
        end
    end
