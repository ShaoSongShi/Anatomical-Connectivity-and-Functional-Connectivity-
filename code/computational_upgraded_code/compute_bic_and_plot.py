...
用法：
  python3 compute_bic_and_plot.py [results_prefix] <prepared.mat> [output_prefix] [--fisher-z]
  results_prefix : 如 'sammon_results_dim_'
"""
...
# ---------------------------------------------------------------------------
# 修改 main 函数签名及内部逻辑
def main(results_prefix, prepared_path, output_prefix, fisher_z=False):
    # results_prefix : 如 'sammon_results_dim_'
    ...
    results = {}
    for name in methods:
        results[name] = {'dims': dims, 'bics': [], 'stresses': [],
                         'ncc': [], 'mse': [], 'mae': []}

    # 逐个维度加载结果文件
    for i, d in enumerate(dims):
        fname = f"{results_prefix}{d}.mat"
        try:
            res = load_mat_auto(fname)
        except FileNotFoundError:
            print(f"警告: 文件 {fname} 不存在，跳过维度 {d}")
            for name in methods:
                for key in ['bics','stresses','ncc','mse','mae']:
                    results[name][key].append(np.nan)
            continue

        # 提取坐标
        for name, keys in methods.items():
            X = np.asarray(res[keys['coords']], dtype=float)
            if X.ndim == 3:
                X = X.squeeze()
            if X.ndim == 1:
                X = X.reshape(-1, 1)
            ...
            results[name]['bics'].append(bic)

            # stress
            stress = float(np.asarray(res[keys['stress']]).flat[0])
            results[name]['stresses'].append(stress)

            print(f"  {name:22s} d={d:2d}: BIC={bic:14.2f} NCC={ncc:.4f} MSE={mse:.6f} MAE={mae:.6f}")
...
if __name__ == '__main__':
    ...
    ap.add_argument("results_prefix", help="结果文件前缀，如 'sammon_results_dim_'")
    ap.add_argument("prepared_mat", help="prepared.mat 路径")
    ap.add_argument("output_prefix", nargs="?", default="sammon")
    ap.add_argument("--fisher-z", action="store_true")
    args = ap.parse_args()
    main(args.results_prefix, args.prepared_mat, args.output_prefix, args.fisher_z)
