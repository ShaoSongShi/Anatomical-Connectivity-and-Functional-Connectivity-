...
% --- 3.2 IRLS 自适应 Sammon MDS ---
fprintf('========== 正在运行 IRLS 自适应 Sammon MDS (c=%.1f) ==========\n', c_irls);
inner_max_iter = 20;
[X_irls, stress_irls] = irls_sammon_mds(D, n, c_irls, 50, tol, inner_max_iter);
...
function [X, stress] = irls_sammon_mds(D, n, c, max_iter, tol, inner_max_iter)
%IRLS_SAMMON_MDS 基于 IRLS 自适应重加权的鲁棒 Sammon MDS
    ...
    for outer = 1:max_iter
        ...
        for inner = 1:inner_max_iter
            % ===== 修正 1：内层距离必须基于当前 X_inner =====
            d_inner = squareform(pdist(X_inner));
            d_inner(d_inner == 0) = eps;
            
            V = diag(sum(W, 2)) - W;
            B = zeros(N);
            B(mask) = -W(mask) .* D(mask) ./ d_inner(mask);  % <--- 修正：d_inner
            B(1:N+1:end) = -sum(B, 2);
            
            % ===== 数值稳定化求解 =====
            lambda_reg = 1e-6;
            
            % ===== 修正 2：右侧必须是 B * X_inner =====
            X_new = (V + lambda_reg * eye(N)) \ (B * X_inner);  % <--- 修正：X_inner
