...
# ====================== 配置路径 ======================
corr_matrix_path = "/home/user/ShenRuihong/bishe/corr_principal_validation/data/coupling_correlation_matrix_syn_mat_ct.csv"
cell_info_path = "/home/user/ShenRuihong/bishe/connectomics_at_cosyne-main/docs/resources/data/v1718_cell_info.csv"
output_csv_path = "/home/user/ShenRuihong/bishe/corr_principal_validation/data/coupling_correlation_matrix_syn_mat_ct_meso.csv"
MAT_X_STRESS  = "/home/user/ShenRuihong/bishe/corr_principal_validation/microns_syn_mat_ct/huber_irls_noMaxD/d5/mds_result_irls_d5.mat"
...
# ==========================================
# 1.2 读取细胞信息：使用 meso_type（核心修改）
# ==========================================
println("正在读取细胞信息（按 meso_type 分类）...")
...
df_cell_info[!, :bodyId_str] = string.(df_cell_info.pt_root_id)  # MICrONS 用 pt_root_id
...
# ====================== 【核心修改】使用 meso_type ======================
valid_mask = .!ismissing.(df_filtered.meso_type) .&
             .!isnothing.(df_filtered.meso_type) .&
             (df_filtered.meso_type .!= "")

df_clean = df_filtered[valid_mask, :]
count_removed = count_before_clean - nrow(df_clean)
println("   因缺失 meso_type 被剔除的细胞数: $count_removed")
println("   最终保留细胞数: $(nrow(df_clean))")
...
# ====================== 【核心修改】提取 meso_type ======================
cell_types = df_clean.meso_type  # 这里从 type → meso_type
...
# ==========================================
# 3. 按 meso_type 重新排序矩阵（分块热图用）
# ==========================================
println("正在按 meso_type 排序分块...")

sort_df = DataFrame(id = ids_matrix, type = cell_types, idx = 1:N)
sort!(sort_df, [:type, :idx])  # 按 meso_type 排序
...
# # ==========================================
# println("正在保存 CSV...")
# df_to_save = DataFrame(bodyId = final_sorted_ids)
...
# ==========================================
# 5. 论文要求归一化：Tr(C)/N = 1（你之前确认的正确步骤）
# ==========================================
# ==========================================
# 先验证：当前矩阵是否已经满足 Tr(C)/N = 1
# ==========================================
current_trace = tr(Corr)                  # 矩阵的迹（对角线和）
current_mean_diag = current_trace / N      # Tr(C)/N
println("🔍 归一化前检查：")
println("   矩阵对角线和 Tr(C) = ", current_trace)
println("   对角线均值 Tr(C)/N = ", current_mean_diag)

# 判断是否已经接近 1（允许微小浮点误差）
is_normalized = isapprox(current_mean_diag, 1.0; atol=1e-4)

if is_normalized
    println("✅ 矩阵已经满足 Tr(C)/N ≈ 1，无需再次归一化")
    C_normalized = copy(Corr)
else
    println("⚠️  矩阵未归一化，执行 Tr(C)/N = 1 缩放")
    scale_val = current_mean_diag
    C_normalized = Corr ./ scale_val
end

# 最后统一处理极小值
C = max.(C_normalized, 1e-10)
# ==========================================
...
# ==========================================
# 📊 诊断：画出 C_clean 热图 + 分布直方图
# ==========================================

# 1. 画 C_clean 热图（看矩阵长什么样）
...
# 3. 打印关键统计（你马上看到真相）
println("\n📊 C_clean 统计信息（真相）：")
println("非对角线均值: ", mean(non_diag))
println("非对角线中位数: ", median(non_diag))
println("最小值: ", minimum(non_diag))
println("最大值: ", maximum(non_diag))
println("小于 0.1 的比例: ", mean(non_diag .< 0.1))
println("大于 0.5 的比例: ", mean(non_diag .> 0.5))
...
valid_indices = 15:(length(cor_vals) - 10)
...
savefig("loglog_fit_scatter.png")
L = (N/ρ)^(1/n)
...
# ==========================================
matwrite(MAT_D, Dict("D" => D))
println("✅ 距离矩阵 D 已保存为 MAT 格式：distance_matrix_for_matlab_AL.mat")
...
savefig(p_D, "D_distribution.png")
println("✅ D分布图已生成！")
...
CSV.write("distance_matrix_stress.csv", DataFrame(X, :auto))
...
savefig(p_scatter, "point_cloud_all_cells.png")
# ==========================================
# 【单独保存：细胞类型图例】
# ==========================================
...
# 保存单独图例
savefig(p_legend_only, "cell_type_legend_only.png")
println("✅ 细胞类型图例已单独保存！")
...
savefig(p_comparison, "corr_comparison.png")
println("✅ 并排对比热图已保存")
...
title!("Cell Type Legend")
savefig(p_legend, "cell_type_legend.png")
println("✅ 细胞类型图例已保存")
